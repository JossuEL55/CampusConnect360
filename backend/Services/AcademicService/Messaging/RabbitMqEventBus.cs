    ...
    ILogger<RabbitMqEventBus> logger) : BackgroundService, IIntegrationEventPublisher
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                consumer.ReceivedAsync += (_, args) => ProcessPaymentConfirmedAsync(args, stoppingToken);
                await consumerChannel.BasicConsumeAsync(
                    RabbitMqNames.Queues.AcademicPaymentConfirmed,
                    autoAck: false,
                    consumer,
                    stoppingToken);

                logger.LogInformation(
                    "Consumiendo la cola {Queue}.",
                    RabbitMqNames.Queues.AcademicPaymentConfirmed);
    ...
    private async Task ProcessPaymentConfirmedAsync(
        BasicDeliverEventArgs args, CancellationToken stoppingToken)
    {
        if (consumerChannel is null) return;
        try
        {
            var envelope = JsonSerializer.Deserialize<EventEnvelope<PaymentConfirmedData>>(
                args.Body.Span,
                JsonOptions) ?? throw new JsonException("Evento PaymentConfirmed vacío.");
            ValidatePaymentConfirmed(envelope);

            await using var scope = scopeFactory.CreateAsyncScope();
            await scope.ServiceProvider.GetRequiredService<AcademicOperations>()
                .PaymentConfirmedAsync(envelope, stoppingToken);
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "PaymentConfirmed inválido o no procesable; se enviará al Dead Letter Exchange.");
    ...
    private async Task<IConnection> EnsureConnectionAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            connection = await factory.CreateConnectionAsync("academic-service", ct);
            return connection;
        }
        finally
        {
    ...
    private static void ValidatePaymentConfirmed(EventEnvelope<PaymentConfirmedData> envelope)
    {
        if (envelope.EventId == Guid.Empty ||
            envelope.EventType != EventTypes.PaymentConfirmed ||
            string.IsNullOrWhiteSpace(envelope.CorrelationId) ||
            envelope.Data.PaymentId == Guid.Empty ||
            (envelope.Data.StudentId == Guid.Empty && envelope.EntityId == Guid.Empty))
        {
            throw new JsonException("El evento PaymentConfirmed no cumple el contrato de integración.");
        }
    }
